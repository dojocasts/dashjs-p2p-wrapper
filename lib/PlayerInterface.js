...
class PlayerInterface {
    ...
    constructor (player, manifestHelper, liveDelay, bufferLevel) {
        this._player = player;
        this._manifestHelper = manifestHelper;
        this._liveDelay = liveDelay;
        this._bufferLevelFromPage = bufferLevel;
    ...
    setBufferMarginLive(bufferLevel) {
        ...
        if (!isNaN(this._bufferLevelFromPage)) {
            console.info("setBufferMarginLive(): buffer level from page is defined, falling back to it");

            this.doSetBuffer(this._bufferLevelFromPage);
            return;
        }
        ...
        this.doSetBuffer(this._dashjsBufferTime);
    }

    doSetBuffer(value) {
        console.info("doSetBuffer(): value=", value);
        this._player.setStableBufferTime(value);
        this._player.setBufferTimeAtTopQuality(value);
        this._player.setBufferTimeAtTopQualityLongForm(value); // TODO: can live be "long form" ?
    }
    ...
    _createOnTrackChangeListener (observer) {
        ...
        return function({ mediaType, streamInfo, oldQuality, newQuality}) {
            ...
            console.info("quality changed(): id=", id, "from", oldQuality, "to", newQuality);
            var tracks = {};
...
console.info("_dispatchInitialOnTrackChange(): id=", this._id);
