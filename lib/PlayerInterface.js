...
import ExternaEvents from './ExternalEvents';

const QUALITY_CHANGE_EVENT = dashjs.MediaPlayer.events.QUALITY_CHANGE_START || dashjs.MediaPlayer.events.QUALITY_CHANGE_REQUESTED;
...
class PlayerInterface {
    ...
    addEventListener (eventName, observer) {
        if (!ExternaEvents.isSupported(eventName)) {
        ...
        this._player.on(QUALITY_CHANGE_EVENT, onTrackChangeListener);
    }
    ...
    removeEventListener(eventName, observer) {
        ...
        this._player.off(QUALITY_CHANGE_EVENT, onTrackChangeListener);
