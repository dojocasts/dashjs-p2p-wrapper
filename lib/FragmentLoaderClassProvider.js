...
function FragmentLoaderClassProvider(wrapper) {
    ...
    function modifyRequestDates(stats, request, traces) {
        const now = Date.now();
        const p2pRequestDuration = stats.p2pDownloaded ? stats.p2pDuration + stats.cdnDuration : 0;

        request.firstByteDate = request.firstByteDate || request.requestStartDate;
        request.requestEndDate = new Date(now);

        if (p2pRequestDuration === 0) {
            return;
        }

        request.requestStartDate = new Date(now - p2pRequestDuration);
        request.firstByteDate = new Date(now - p2pRequestDuration + computeEstimatedRTT(p2pRequestDuration));

        const requestStartDateTime = request.requestStartDate.getTime();
        const firstByteDateTime = request.firstByteDate.getTime();
        const latency = firstByteDateTime - requestStartDateTime;
        if (latency < DASH_INTERNAL_CACHE_LATENCY) {
            request.requestStartDate.setTime(requestStartDateTime - (DASH_INTERNAL_CACHE_LATENCY - latency));
        }

        compensateTraceTimestamps(traces, p2pRequestDuration);
    }
