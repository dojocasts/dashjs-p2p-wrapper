...
function FragmentLoaderClassProvider(wrapper) {
    ...
    this.SRFragmentLoader = function (config) {
        ...
        function _getHeadersForRequest(request) {
            let xhrHeaders = new XHRHeaders();
            xhrHeaders = requestModifier.modifyRequestHeader(xhrHeaders);

            let headers = {};
            for (let header of Object.keys(xhrHeaders.headers)) {
                headers[header] = xhrHeaders.headers[header];
            }
        ...
        function _getSRRequest(request, headers) {
            ...
            if (mediaPlayerModel.getXHRWithCredentials()) {
                srRequest.withCredentials = true;
            }
