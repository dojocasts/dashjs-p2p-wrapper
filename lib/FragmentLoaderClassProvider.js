...
function FragmentLoaderClassProvider(wrapper) {
    ...
    function modifyRequestDates(stats, request, traces) {
        ...
        request.requestStartDate = new Date(now - p2pDownloadDuration);

        // if latency is defined, we use its value considering 50ms dash.js cached response treshold
        const latency = stats.latency || Math.min(DASH_INTERNAL_CACHE_LATENCY, (request.requestEndDate - request.requestStartDate) / 2);
        request.firstByteDate = new Date(request.requestStartDate.getTime() + latency);
        ...
        function _getSegmentViewForRequest(request) {
            if (request.type !== "InitializationSegment" &&
                ...
                let trackView = new TrackView({
                    ...
                    bitrate: request.mediaInfo.bitrateList[request.quality].bandwidth,
                });
